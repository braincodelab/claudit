...
df_reftable = pd.read_excel(
    "/Users/paulryu/BrainCodeLab/DICOM-ReAn/MISTIE_III_Link_File.xlsx"
)
# Set study path.
path_study = (
    "/Volumes/LaCie-1626/DATA 2/Image Archive 2/MISTIE III/Anonymized_restructured/"
)
# Set export path.
...
def find_datetime_i(files, pt):
    ...
    for f in files:
        dataset = pydicom.dcmread(
            f, specific_tags=[0x00080022, 0x00080023, 0x00080032, 0x00080033]
        )
        ...
        try:
            datetimelist.append(
                datetime.strptime(
                    dataset.AcquisitionDate + dataset.AcquisitionTime, "%Y%m%d%H%M%S.%f"
                )
            )
        except ValueError:
            datetimelist.append(
                datetime.strptime(
                    dataset.AcquisitionDate + dataset.AcquisitionTime + ".0",
                    "%Y%m%d%H%M%S.%f",
                )
            )
    ...
    print(
        timestamp(),
        "Successfully found the earliest scan for",
        pt,
        ":",
        datetime_initial,
    )
...
for x in folders_pt:
    ...
    for count, value in enumerate(files_dcm):
        ...
        datetime_n = datetime.strftime(
            datetime(2000, 1, 1, 0, 0, 0, 0) + datetime_d, "%Y%m%d%H%M%S.%f"
        )
        ...
        count_study, count_series, count_scan = countnum(
            count_study, count_series, count_scan, count, value, files_dcm
        )
        count_study_str, count_series_str, count_scan_str = (
            str(count_study),
            str(count_series),
            str(count_scan),
        )
        ...
        ds.file_meta.ImplementationVersionName = "PYDICOM " + ".".join(
            str(x) for x in __version_info__
        )
        ds.file_meta.MediaStorageSOPInstanceUID = (
            uid_prefix + count_study_str + "." + count_series_str + "." + count_scan_str
        )
        ds.SOPInstanceUID = (
            uid_prefix + count_study_str + "." + count_series_str + "." + count_scan_str
        )
